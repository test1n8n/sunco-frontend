...
function ReportView({ report }: { report: Report }) {
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div className="flex flex-wrap items-start gap-3 pb-2 border-b border-border">
        <div>
      ...
      <div className="bg-card border border-border border-l-2 border-l-accent rounded p-5">
        <h3 className="text-text-dim font-semibold text-xs uppercase tracking-widest mb-3">Market Summary</h3>
        <p className="text-text-primary text-sm leading-relaxed">{report.market_summary}</p>
      </div>

      {/* Key News */}
      <div>
        <h3 className="text-text-dim font-semibold text-xs uppercase tracking-widest mb-3">Key News</h3>
        {filteredNews.length === 0 ? (
          <p className="text-text-dim text-sm">No high/medium relevance news.</p>
        ) : (
      ...
      {/* Macro Signals */}
      <div>
        <h3 className="text-text-dim font-semibold text-xs uppercase tracking-widest mb-3">Macro Signals</h3>
        <MacroSignalsTable signals={report.macro_signals} />
      </div>

      {/* Outlooks */}
      <div>
        <h3 className="text-text-dim font-semibold text-xs uppercase tracking-widest mb-3">Market Outlook</h3>
      ...
      <div className="bg-card border border-border rounded p-4">
        <h3 className="text-text-dim font-semibold text-xs uppercase tracking-widest mb-2">SAF Note</h3>
      ...
      {report.broker_notes && (
        <div className="bg-card border border-border rounded p-5">
          <h3 className="text-text-dim font-semibold text-xs uppercase tracking-widest mb-2">Broker Notes</h3>
      ...
      {selectedReport ? (
        <div>
          <button
            ...
            className="flex items-center gap-2 text-accent text-xs font-semibold hover:underline mb-5 uppercase tracking-widest"
          >
            ← Back to Archive
          </button>
        ...
        <div className="bg-card border border-border rounded overflow-hidden">
          <div className="px-6 py-4 border-b border-border">
            <h2 className="text-text-dim font-semibold text-xs uppercase tracking-widest">Report Archive</h2>
            <p className="text-text-secondary text-sm mt-1">Select a date to view the full report</p>
          </div>

          {listLoading ? (
            <Spinner />
          ) : dateList.length === 0 ? (
            <div className="text-center py-12 text-text-dim text-sm">No archived reports found.</div>
          ) : (
              ...
              {dateList.map((item) => (
                <li key={item.id}>
                  <button
                    ...
                    className="w-full text-left px-6 py-4 hover:bg-surface transition-colors flex items-center justify-between group"
                  >
                    <div>
                      <p className="font-semibold text-text-primary text-sm group-hover:text-accent transition-colors">
                        {formatDate(item.report_date)}
                      </p>
                      <p className="text-xs text-text-dim mt-0.5 font-mono">{item.report_date}</p>
                    </div>
                    <span className="text-accent text-xs font-semibold tracking-widest uppercase group-hover:underline">
                      View →
                    </span>
                  </button>
                </li>
